...
class NetworkAdapterSelection(Screen, HelpableScreen):
    ...
    def __init__(self, session):
        ...
        self['DefaultInterfaceAction'] = HelpableActionMap(self, 'ColorActions', {'yellow': (self.setDefaultInterface, [_('Set interface as default Interface'), _('* Only available if more than one interface is active.')])})
...
#            self.session.openWithCallback(self.InstallPackageFailed, MessageBox, _('Sorry feeds are down for maintenance, please try again later.'), type=MessageBox.TYPE_INFO, timeout=10, close_on_any_key=True)
    ...
    def installComplete(self, result = None, retval = None, extra_args = None):
        msg = _('Enigma2 will be now hard restarted to activate service.\nDo You want restart enigma2 now?')
...
class NetworkNfs(Screen):
    ...
    def doRemove(self, callback, pkgname):
        self.message = self.session.open(MessageBox, _('please wait...'), MessageBox.TYPE_INFO, enable_input=False)
        self.message.setTitle(_('Removing Service'))
        self.Console.ePopen('/usr/bin/opkg remove ' + pkgname + ' --force-depends --force-remove --autoremove', callback)
...
class NetworkSamba(Screen):
    ...
    def __init__(self, session):
        ...
        self.service_samba = basegroup + '-smbfs'
        self.service_name = self.service_samba + ' ' + basegroup + '-smbfs-server' + ' ' + 'samba'
        self.onLayoutFinish.append(self.InstallCheck)
    ...
    def UninstallCheck(self):
        self.service_name = self.service_name + ' ' + basegroup + '-smbfs-client' + ' ' + basegroup + '-smbfs-server'
        self.Console.ePopen('/usr/bin/opkg list_installed ' + self.service_name, self.RemovedataAvail)
